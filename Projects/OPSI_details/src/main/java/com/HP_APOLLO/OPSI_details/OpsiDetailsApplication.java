...
public class OpsiDetailsApplication {
	...
	private static Function<HttpHeaders, HttpEntity<?>> getTokenRequest=t -> {
		  ...
		  String>(); body.add("grant_type", "client_credentials");
...
static private BiFunction<HttpEntity<?>, RestTemplate,ResponseEntity<String>> getOpsiDetails= (t,OpsiTemplate) ->{
	  ...
	  "https://test.reltio.com/reltio/api/2TBErW89nsMdXlt/entities?filter=" +
	  "(equals(type,'configuration/entityTypes/OPSI') and " +
	  "equals(attributes.ID,"+OpsiId+")),preserveURIs", HttpMethod.GET,t,String.class);
	...
	static BiPredicate<Environment, ResponseEntity<String>> saveOpsiToFile = (t, opsiResponseEntity) -> {
		...
		String opsiFileName=String.format("/Users/BA20382584/eclipse-workspace/OPSI_details/src/main/resources/ID-%s.json", 
					  OpsiId);
		  ...
		  try { 
			  json =
		  Obj.readValue(opsiResponseEntity.getBody(),Object.class); } catch
		  (JsonProcessingException e1) { json=null; e1.printStackTrace(); }
	...
	RestTemplate getOpsiTemplate() {
	  ...
	  @Bean 
	  CommandLineRunner saveOpsiData(RestTemplate OpsiTemplate) throws
	  Exception { return args -> {
	  
	  String token=getToken.andThen(getTokenCode).apply(getCredentials.
			  andThen(getEncodedCredentials).
			  andThen(getTokenAuthorisationHeaders).andThen(getTokenRequest)
	  ...
	  for(int i=1; ;i++) {
		  ...
		  if(checkIdExists.test(OpsiId)){
			  ...
			  ResponseEntity<String> opsiResponseEntity=getOpsiDetails.apply(getOpsiAuthorisationHeader.
						 andThen(getOpsiRequest).apply(token),OpsiTemplate);
...
context.close();
	...
	/*
	 * @Bean CommandLineRunner printToken() throws Exception{
	 * 
	 * return args ->{ ResponseEntity<Token> responseEntity=getCredentials.
	 * andThen(getEncodedCredentials).
	 * andThen(getTokenAuthorisationHeaders).andThen(getTokenRequest).
	 * andThen(getToken).apply(environment);
	 * 
	 * log.info(responseEntity.getBody().getAccess_token());
	 * log.info("Token code: "+Base64.decodeBase64(responseEntity.getBody().
	 * getAccess_token()));
	 * log.info("Token type: "+responseEntity.getBody().getToken_type().toString());
	 * };
	 * 
	 * }
	 */
